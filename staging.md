...
<tr class="stg-row-fail">
  ...
  <div class="stg-limits">
    <strong>Resolved.</strong> (1) ALMA Table 1 values confirmed: D3 = 42.3 AU, D4 = 50.3 AU — tightens P1–P3 match to 0.08 AU. (2) Mass uncertainty tested across 11 configurations — 9/11 within 4 AU, result is robust. (4) Hill sphere clearance at 24.2 AU is +7.2 AU — Hill overlap does not explain P1–P2 non-match.
    <br><br>
    <strong>Open.</strong> (3) The full 2D azimuthal average shifts the P1–P3 parity surface by +6.3 AU — the primary unresolved concern. Note however that gap formation is resonance-driven (Lindblad/corotation), not time-averaged-field-driven; the radial-line (L1 analog) formula may be physically correct for this application, in which case the azimuthal average is the wrong model. This distinction is unresolved. The P1–P2 non-match has no quantitative explanation.
    <br><br>
    <strong>Cross-system sweep (DSHARP + HL Tau, 7 systems, Analysis I.6).</strong> Running the same planet–planet parity framework across all DSHARP systems with ≥ 2 planet candidates: 11 parity surfaces computed, 12 novel gaps across 3 testable systems. <strong>Only HL Tau produces hits</strong> — both tight (Δ &lt; 1.5 AU). No hits in AS 209 (P1–P2 parity at 44.0 AU vs. novel gaps at 24, 35, 61, 90 AU), TW Hya (parity at 29.5 AU vs. gaps at 1, 4, 47 AU), or IM Lup (parity at 41.6 AU vs. gap at 117 AU). HD 163296 P2–P3 parity lands at 63.7 AU vs. ring at 67 AU (Δ = 3.3 AU — narrowly outside 3 AU threshold). Null model hit probability per testable system averages 0.18 under random placement. The pattern does not replicate across the DSHARP sample in its current form. Two candidate explanations: (a) the HL Tau result is coincidental; (b) the mechanism requires specific geometry — three widely-spaced, comparably-massed planets — not present in the other tested systems. Further testing requires better planet identification across DSHARP (many systems have contested or incomplete planet census).
  </div>
